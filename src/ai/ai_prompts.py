...
- results: 搜索结果列表,每个结果包含:
  ...
  - inLibrary: 该源是否已存在于本地弹幕库中 (重要! true 表示此作品之前已用该源入库)
  - matchesRecognitionRule: 该结果是否命中用户的识别词规则 (身份校正信息,见下方说明)

**关于 matchesRecognitionRule (身份校正,非排序依据)**:
用户可配置"识别词规则",把源站标题映射为入库作品名。例如规则把源站的"说唱巅峰对决2026"
映射为"中国新说唱 第九季"。若某结果 matchesRecognitionRule=true,表示**该结果经用户规则
转换后,正是用户想找的作品**——即使它的字面 title 与 query.title 看起来不同,也应视为高度匹配,
不要因字面差异而排除它。query 中可能附带 recognition_hint 文字,进一步说明映射意图。
**注意**: 此标记仅用于帮助你正确理解条目的真实身份,**不改变下方的排序优先级**。

**匹配规则** (按优先级排序):
1. **精确标记绝对优先**: 如果某个结果的 isFavorited=true,必须优先选择它 (除非类型或季度明显不匹配)
2. **库内已有源优先**: 如果有结果的 inLibrary=true,应优先选择它们 (这些源此作品已入库,复用最稳定,避免同一作品不同集使用不同源)。
   - 若存在多个 inLibrary=true 的结果: 其中有 isFavorited=true 的选被标记的;否则选**列表中排序靠前**的 (结果列表已按源优先级排好序,靠前=优先级更高)
3. **标题相似度**: 优先匹配标题相似度最高的条目 (注意: matchesRecognitionRule=true 的结果应视为与 query 高度相似,优先于纯字面相似度)
4. **季度严格匹配**: 如果指定了季度,必须严格匹配
5. **类型匹配**: 电视剧优先匹配 tv_series,电影优先匹配 movie
6. **年份接近**: 优先选择年份接近的
