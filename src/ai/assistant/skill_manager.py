...
功能：
1. 启动时扫描 config/skills/ 并解析所有 SKILL.md
...
class Skill:
    ...
    content: str = ""  # SKILL.md 正文（去掉 frontmatter 后）
...
class SkillManager:
    ...
    def load_all(self) -> None:
        """扫描 config/skills/ 加载所有 SKILL.md。"""
        base_dir = get_skills_base_dir()
        if base_dir is None:
            logger.warning("技能目录未初始化，跳过加载")
            return
        if not base_dir.exists():
            logger.info("技能目录不存在，创建空目录")
        ...
        for skill_dir in base_dir.iterdir():
            ...
            try:
                ...
                logger.info(f"加载技能: {skill.skill_id} v{skill.version} ({skill.name})")
            except Exception as e:  # noqa: BLE001
                logger.error(f"解析 {skill_file} 失败: {e}", exc_info=True)

        self._loaded = True
        logger.info(f"技能加载完成，共 {len(self._skills)} 个")

    def _parse_skill_file(self, skill_id: str, file_path: Path) -> Skill:
        ...
        if not match:
            # 无 frontmatter，整个当正文
            return Skill(skill_id=skill_id, name=skill_id, content=text.strip(), file_path=file_path)
    ...
    def toggle_skill(self, skill_id: str, enabled: bool) -> Skill:
        """启用/停用技能（更新 frontmatter enabled 字段）。"""
        skill = self._skills.get(skill_id)
        if not skill:
            raise ValueError(f"技能 {skill_id} 不存在")
